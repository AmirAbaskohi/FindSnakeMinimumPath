...
def manhatanDistaance(point1, point2):
    manhatanDistance = 0
    if abs(point1[0] - point2[0]) > HEIGHT - abs(point1[0]- point2[0]):
        manhatanDistance += HEIGHT - abs(point1[0]- point2[0])
    else:
        manhatanDistance += abs(point1[0]- point2[0])
    if abs(point1[1] - point2[1]) > WIDTH - abs(point1[1] - point2[1]):
        manhatanDistance += WIDTH - abs(point1[1] - point2[1])
    else:
        manhatanDistance += abs(point1[1] - point2[1])
...
class State:
    ...
    def hScore(self):
        h = 0
        seedsCopy = copy.copy(self.seeds)
        point1 = (self.snake.headY, self.snake.headX)
        minimumMD = WIDTH + HEIGHT
        for i in range(len(seedsCopy)):
            for key in seedsCopy:
                if seedsCopy[key] != 0:
                    if manhatanDistaance(point1, key) < minimumMD:
                        minimumMD = manhatanDistaance(point1, key)
                        lastKey = key
            seedsCopy[lastKey] = 0
            point1 = lastKey
            h += minimumMD
...
def aStarSearch():
    ...
    global MOVES, frontier, exploredStates, numberOfStates, numberOfStates, numberOfDifferentStates
    start = time.time()
